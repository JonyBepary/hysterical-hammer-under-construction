...
import os
import sys
from core import tartget_dir_func
...
def arg_procces(arg, key):
  print('start')
  if key == '--strike':
    command_processor.strike(tartget_dir_func(os.getcwd()))
  elif key == '--check-file':
    if arg['<FILE>']:
    ...
    print('True!!!')
    if arg['<DIR>']:
      command_processor.only_binary(tartget_dir_func(os.getcwd()))
    else:
      pass

  elif key == '--only-text':
    if arg['<DIR>']:
      command_processor.only_text(tartget_dir_func(os.getcwd()))
    else:
      pass

  elif key == '--unoptimize-list':
    if arg['<DIR>']:
      command_processor.unoptimize_list(tartget_dir_func(os.getcwd()))
    else:
      pass

  elif key == '--setup':
    command_processor.setup(tartget_dir_func(os.getcwd()))
